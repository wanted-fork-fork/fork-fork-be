...
@Service
class KakaoAuthService(val kaKaoAuthFeignClient: KaKaoAuthFeignClient, val kaKaoApiFeignClient: KaKaoApiFeignClient, val kakaoAuthProperties: KakaoAuthProperties) {
    fun getKakaoUserInfo(code: String): KakaoUserInfoResponse {
        val tokenResponse = getAccessToken(code)
        val headers = mapOf(CONTENT_TYPE to APPLICATION_FORM_URLENCODED_VALUE, AUTHORIZATION to "${tokenResponse.tokenType} ${tokenResponse.accessToken}")
        return kaKaoApiFeignClient.getKakaoUserInfo(headers)
    }

    fun getAccessToken(code: String) = kaKaoAuthFeignClient.getAccessToken(
    ...
    fun logout(accessToken: String, targetKakaoId: Long) {
